...
class ModelVendorVendor extends Model {
	...
	public function addVendor($data) {
		...
		$message = "Welcome ".$data['firstname'].' '.$data['lastname']. "<br/><br/>";
		$message .= "Following is the link to your affilitate dash board". "<br/>";
		$message .= HTTP_CATALOG.'index.php?route=vendor/account'. "<br/><br/>";
		$message .= 'Your login details are'."<br/>";
		$message .= 'Email: '.$data['email']."<br/>";
		$message .= 'Password: '.$data['password']."<br/><br/>";
		$message .= "Thanks" . "<br/>";
		$message .= $this->config->get('config_name');
		$subject = $this->config->get('config_name').'- Affiliate Login Details';				$mail = new PHPMailer(true);		$mail->IsSMTP();		$mail->SMTPAuth = true; 		$mail->Port = '25';		$mail->Host = 'mail.bungaindo.com';		$mail->Username = 'admin@bungaindo.com'; 		$mail->Password = 'kebunrayabogor1'; 		$mail->SMTPAuth = true; 		$mail->AddReplyTo($this->config->get('config_email'));		$mail->From = $this->config->get('config_email');		$mail->FromName = $this->config->get('config_name');		$mail->AddAddress($data['email']);		$mail->Subject = (html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));		$mail->AltBody = "To view the message, please use an HTML compatible email viewer!"; 		$mail->WordWrap = 80; 		$mail->MsgHTML(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));		$mail->IsHTML(true);		$mail->Encoding = 'base64';		if(!$mail->Send()) {						echo "Mailer Error: " . $mail->ErrorInfo;exit;				} else {										}		
		/* $mail = new Mail();
