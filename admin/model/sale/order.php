...
class ModelSaleOrder extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		//-- comented by gaurav not to change total price after admin edit as we are separated orders		 
		/*$this->db->query("UPDATE `" . DB_PREFIX . "order` SET total = '" . (float)$total . "', affiliate_id = '" . (int)$affiliate_id . "', commission = '" . (float)$commission . "' WHERE order_id = '" . (int)$order_id . "'"); */
		
		//------------added by gaurav to save payemnt details
		$paymentDate = strtotime($data['payment_date']);
		$date = date('Y-m-d',$paymentDate);
		if($data['payment_to'] != '' && $data['payment_from'] != '' && $paymentDate != '' && $data['account_name'] != '')
		{
			 $SQLPayment = "UPDATE ". DB_PREFIX ."order_customer_payment set payment_to = '".$data['payment_to']."',payment_from= '".$data['payment_from']."',account_name= '".$data['account_name']."',total_amount= '".$data['total_amount_paid']."',payment_date= '".$date."', status ='confirm' WHERE order_id = '".(int)$order_id."' ";
	...
	//------------added by Gaurav on 3rd-june-2013 to get order status from status id for sending email to vendor if admin approv order
	public function getOrderStatus($order_id){
		$sql = "SELECT `name` FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = '" . (int)$order_id . "'";
		$query = $this->db->query($sql);
		return $query->row;	
	}	
													
	//----------- added by Gaurav on 10-june-13 to assign order to vendor
	public function updateOrderVendor($data){
		$order_status_query = $this->db->query("SELECT order_status_id FROM " . DB_PREFIX . "order_status WHERE name LIKE  '%Assigned%' AND language_id = '" . (int)$this->config->get('config_language_id')  . "'");
		$intOrderStatusId = $order_status_query->row['order_status_id'];
		
		$sqlUpdate = "UPDATE ".DB_PREFIX."order o SET o.vendor_id = ".(int)$data['vendor_id'].", o.order_status_id = ".(int)$intOrderStatusId."  WHERE o.order_id = ".(int)$data['order_id']." ";
		$this->db->query($sqlUpdate);
	}			
	
	/**
	* @param $orderId
	* @return payment details of order with given order id
	*/
	public function getPaymentDetails($orderId)
	{
		$sql = "SELECT * FROM " . DB_PREFIX . "order_customer_payment WHERE order_id =".(int)$orderId." ";
		$query = $this->db->query($sql);
		if($query->num_rows > 0)
		{
			return $query->row;
