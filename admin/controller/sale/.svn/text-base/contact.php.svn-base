...
class ControllerSaleContact extends Controller {
	...
	public function send() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			//--------- added by Gaurav on 4-06-2013 to get emails from newsletter table for newsletter subscribtions only.
						$newsletterSubscribers = $this->model_sale_customer->getNewsletterSubscribers();
						foreach($newsletterSubscribers AS $user)
						{
