...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_upselling'])) {
				$url .= '&filter_upselling=' . $this->request->get['filter_upselling'];
			}
  	...
  	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			...
			if (isset($this->request->get['filter_upselling'])) {
				$url .= '&filter_upselling=' . urlencode(html_entity_decode($this->request->get['filter_upselling'], ENT_QUOTES, 'UTF-8'));
			}
  	...
  	protected function getList() {				
		...
		if (isset($this->request->get['filter_upselling'])) {
			$filter_upselling = $this->request->get['filter_upselling'];
		} else {
			$filter_upselling = null;
		}
		...
		$data = array(
			...
			'filter_upselling' => $filter_upselling,
			'sort'            => $sort,
			'order'           => $order,
		...
		$this->data['sort_upselling'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.upselling' . $url, 'SSL');
		...
		$this->data['filter_upselling'] = $filter_upselling;
  	...
  	protected function getForm() {
		...
		$this->data['entry_upselling'] = $this->language->get('entry_upselling');
		...
		if (isset($this->request->post['upselling'])) {
      		$this->data['upselling'] = $this->request->post['upselling'];
    	} elseif (!empty($product_info)) {
			$this->data['upselling'] = $product_info['upselling'];
		} else {
      		$this->data['upselling'] = '';
    	}								
			
		$locationArr = $this->model_catalog_product->getProductLocation($this->request->get['product_id']);			$strLocation = '';
		foreach($locationArr AS $location)
		{
			$strLocation .= ','.$location['location_name'];
		}
		$strLocation = trim($strLocation,',');
		$this->data['location']  = $strLocation;
		...
		if (isset($this->request->post['watermark'])) {
      		$this->data['watermark'] = $this->request->post['watermark'];
    	} elseif (!empty($product_info)) {
			$this->data['watermark'] = $product_info['watermark'];
		} else {
      		$this->data['watermark'] = '';
    	}	
		...
		if (isset($this->request->post['shippingMethod'])) {
      		$this->data['shippingMethod'] = $this->request->post['shippingMethod'];
    	} elseif (!empty($product_info)) {
      		$this->data['shippingMethod'] = $product_info['shippingMethod'];
    	} else {
			$this->data['shippingMethod'] = 'Shipped in a Gift Box';
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id']) || isset($this->request->get['filter_upselling'])) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');
			...
			//------- added by gaurav on 30-05-2013 for filtering product on the basis of category
			if (isset($this->request->get['filter_category_id'])) {
				$filter_category_id = $this->request->get['filter_category_id'];
			} else {
				$filter_category_id = '';
			}
			// --  End of filter added by gaurav on 30-05-2013 
