...
class ControllerCatalogCategory extends Controller { 
	...
	protected function getForm() {
		...
		$this->data['entry_highlighted_product'] = $this->language->get('entry_highlighted_product');
		...
		if(isset($category_info['highlight_product_id']) && $category_info['highlight_product_id'] != 0)
		{
			$this->data['highlight_product_id'] = $category_info['highlight_product_id'];
		
			$this->load->model('catalog/product');
			$products = $this->model_catalog_product->getProductDescriptions($category_info['highlight_product_id']);
			$productName = '';
			...
			$this->data['highlight_product'] = $productName;
		}
		else
		{
			$this->data['highlight_product_id']  = '';
			$this->data['highlight_product'] = '';
		}
		...
		if (isset($this->request->post['bannerImage'])) {
			$this->data['bannerImage'] = $this->request->post['bannerImage'];
		} elseif (!empty($category_info)) {
			$this->data['bannerImage'] = $category_info['bannerImage'];
		} else {
			$this->data['bannerImage'] = '';
		}
		
		if (isset($this->request->post['bannerImage']) && file_exists(DIR_IMAGE . $this->request->post['bannerImage'])) {
			$this->data['bannerImageThumb'] = $this->model_tool_image->resize($this->request->post['bannerImage'], 100, 100);
		} elseif (!empty($category_info) && $category_info['bannerImage'] && file_exists(DIR_IMAGE . $category_info['bannerImage'])) {
			$this->data['bannerImageThumb'] = $this->model_tool_image->resize($category_info['bannerImage'], 100, 100);
		} else {
			$this->data['bannerImageThumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
		...
		if (isset($this->request->post['watermark'])) {
			...
			$this->data['watermark'] = $category_info['watermark'];
		} else {
			$this->data['watermark'] = 0;
		}
