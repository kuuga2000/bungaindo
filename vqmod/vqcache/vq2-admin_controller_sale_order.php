	...
	private $error = array();
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$order_Details = $this->model_sale_order->getOrderStatus($this->request->post['order_status_id']);
			
			if(isset($this->request->post['orderChanged']) && $this->request->post['orderChanged'] == 1 && ($order_Details['name'] == 'Approved' || $order_Details['name'] == 'approved'))
			{
				$shippingCity = $this->request->post['shipping_city'];
				$this->load->model('vendor/vendor');
				$this->load->model('tool/image');
				$vendorsArr = $this->model_vendor_vendor->getVendorByCity($shippingCity);
				...
				$subject = $storeDetails['store_name'].'- New order approved.';
				$template = new Template();
				$orderId = $this->request->get['order_id'];
				foreach($vendorsArr AS $vendor)
				{
					if((isset($vendor['firstname']) && $vendor['firstname'] != '') || (isset($vendor['lastname']) && $vendor['lastname'] != ''))
					{
						$strVendorName = $vendor['firstname'].' '.$vendor['lastname'];	
					}
					else
					{
						$strVendorName = 'Vendor';
					}
					$template->data['strVendorName'] = $strVendorName;
					$template->data['shippingCity'] = $shippingCity;
					$template->data['text_product'] = 'Product name';
					$template->data['text_quantity'] = 'Quantity';
					$template->data['text_price'] = 'Price';
					$template->data['products'] = $this->request->post['order_product'];
					$template->data['store_name'] = $storeDetails['store_name'];
					...
					{
						$template->data['image']  = $this->model_tool_image->resize($this->config->get('config_logo'), 100, 100);		
					} else {
						$template->data['image']  = $this->model_tool_image->resize('no_image.jpg', 100, 100);
					}
					$template->data['store_url'] = $storeDetails['store_url'];
					
					if (file_exists(DIR_TEMPLATE . '/mail/vendorEmail.tpl')) 			{
					$html = $template->fetch('/mail/vendorEmail.tpl');
					} 
					else
					{
					...
					$mail->setTo($vendor['email']);
					$mail->setFrom($this->config->get('config_email'));
					$mail->setSender($storeDetails['store_name']);
				...
				$getCustomer = "SELECT c.email AS email, c.firstname AS firstname, c.lastname AS lastname FROM " . DB_PREFIX . "customer c INNER JOIN " . DB_PREFIX . "order o ON c.customer_id = o.customer_id WHERE o.order_id =  ".(int)$this->request->get['order_id']." ";
		
				$res = $this->db->query($getCustomer);
				$customer = $res->row;
				$subject = $storeDetails['store_name'].'- Order approved.';
				$html = "Dear ".$customer['firstname']." ".$customer['lastname'];
				$html .= '<br><br>';
				$html .= "Your order  has been approved by admin<br>ORDER ID: ".$orderId;
			...
			{
				$orderId = $this->request->get['order_id'];
				$subject = 'Bunga Indo - Order cancelled.';
				$html = "Dear ".$this->request->post['firstname']." ".$this->request->post['lastname'];
				$html .= '<br><br>';
				$html .= "Your order  has been cancelled by admin<br>ORDER ID: ".$orderId;
				$html .= "<br><br>Thank You<br>Bunga Indo";
				$mail = new Mail(); 
				$mail->protocol = $this->config->get('config_mail_protocol');
				$mail->parameter = $this->config->get('config_mail_parameter');
				$mail->hostname = $this->config->get('config_smtp_host');
  	...
  	protected function getList() {
		...
		$this->data['delete'] = $this->url->link('sale/order/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
                $this->data['despatch'] = $this->url->link('sale/despatch/despatch', 'token=' . $this->session->data['token'], 'SSL');
    	...
    	foreach ($results as $result) {
			...
			$getPayment = $this->db->query('SELECT status from oc_order_customer_payment WHERE order_id = '.(int) $result['order_id'].' ');
			$res = $getPayment->row;
...
$this->data['button_pdf_invoice'] = $this->language->get('button_pdf_invoice');
  	...
  	public function getForm() {
		...
		$this->data['button_update_vendor'] = $this->language->get('button_update_vendor');
		...
		//-------------- Getting payment confirmation values
		$paymentDetails = $this->model_sale_order->getPaymentDetails($this->request->get['order_id']);
		...
		*/
		if (isset($this->request->post['payment_to'])) {
			...
			$this->data['payment_to'] = $paymentDetails['payment_to'];
		} else {
		...
		if (isset($this->request->post['payment_from'])) {
			...
			$this->data['payment_from'] = $paymentDetails['payment_from'];
		} else {
      		$this->data['payment_from'] = '';
    	}	
		
		if (isset($this->request->post['account_name'])) {
      		$this->data['account_name'] = $this->request->post['account_name'];
    	} elseif (!empty($paymentDetails)) { 
			$this->data['account_name'] = $paymentDetails['account_name'];
		} else {
		...
		if (isset($this->request->post['payment_date'])) {
			...
			$this->data['payment_date'] = $paymentDetails['payment_date'];
		} else {
      		$this->data['payment_date'] = '';
    	}	
		$payment_timestatmp = strtotime($this->data['payment_date']);
		if( $payment_timestatmp == '')
		{
			$this->data['payment_date'] = '';
		}
		else
		{
			$payment_date = date('d-m-Y',$payment_timestatmp) ;
			$this->data['payment_date'] = $payment_date;
		}
		
		if (isset($this->request->post['total_amount_paid'])) {
  		$this->data['payment_total'] = $this->request->post['total_amount_paid'];
    	} elseif (!empty($order_info)) { 
			$this->data['payment_total'] = $order_info['total'];
		} else {
		...
		foreach ($order_products as $order_product) {
			...
			$this->data['order_products'][] = array(
				...
				'shippingMethod' => $order_product['shippingMethod'],
				...
				'delivery_date'           => $order_product['delivery_date']
			);
		}
		...
		$this->load->model('vendor/vendor');
		$this->data['vendors'] = $this->model_vendor_vendor->getAllVendors();

		if (isset($this->request->post['vendor_id'])) {
      		$this->data['vendor_id'] = $this->request->post['vendor_id'];
		} elseif (!empty($order_info['vendor_id']) && $order_info['vendor_id'] != 0) { 
			$this->data['vendor_id'] = $order_info['vendor_id'];
		} else {
      		$this->data['vendor_id'] = '';
    	}
...
$this->data['button_pdf_invoice'] = $this->language->get('button_pdf_invoice');
			...
			$this->data['breadcrumbs'][] = array(
				...
				'separator' => ' :: '
                        ...
                        $this->data['despatch'] = $this->url->link('sale/despatch/despatch', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
...
//echo "<pre>";print_r($order_info); die();
				$this->data['orders'][] = array(
					...
					'customer_name'		 => ucfirst($order_info['shipping_firstname'])." ".ucfirst($order_info['shipping_lastname']),
...
//-------- function added by Gaurav on 10-june-13 to assign order to vendor from admin section	
	public function assignOrderToVendor()
	{
		$json['success'] = 1;
		$json['message'] = 'Order has been assigned to vendor successfully.';
		$intOrderId = $this->request->get['order_id'];
		$intVendorId = $this->request->get['vendor_id'];
		$data = array();
		$data['order_id'] = $intOrderId;
		$data['vendor_id'] = $intVendorId;
		$this->load->model('sale/order');
		//----------- assigning order to vendor
		$this->model_sale_order->updateOrderVendor($data);
		
		$this->load->model('vendor/vendor');
		$this->load->model('tool/image');
		$vendorsArr = $this->model_vendor_vendor->getVendor($intVendorId);

		$orderDetails = $this->model_sale_order->getOrder($intOrderId);
		$subject = $orderDetails['store_name'].'- New order Assigned.';
		$template = new Template();
		// sending an email to vendor to whome order is assigned
		foreach($vendorsArr AS $vendor)
		{
			if((isset($vendor['firstname']) && $vendor['firstname'] != '') || (isset($vendor['lastname']) && $vendor['lastname'] != ''))
			{
			...
			$template->data['text_quantity'] = 'Quantity';
			$template->data['text_price'] = 'Price';
			$template->data['order_id'] = $intOrderId;
			$template->data['shipping_city'] = $orderDetails['shipping_city'];
			$template->data['shipping_postcode'] = $orderDetails['shipping_postcode'];
			$template->data['customer_name'] = $orderDetails['shipping_firstname'].' '.$orderDetails['shipping_lastname'];
			$template->data['shipping_address'] = $orderDetails['shipping_address_1'];
			$template->data['total'] = $orderDetails['total'];
			
			
			$template->data['products'] = $this->model_sale_order->getOrderProducts($intOrderId);
			$template->data['store_name'] = $orderDetails['store_name'];
			...
			$template->data['store_url'] = $orderDetails['store_url'];
			
			if (file_exists(DIR_TEMPLATE . '/mail/assignOrdertoVendor.tpl')) 			{
			$html = $template->fetch('/mail/assignOrdertoVendor.tpl');
			} 
			else
			{
				$html = 'Please check the email template';	
			...
			$mail->setTo($vendor['email']);
			$mail->setFrom($this->config->get('config_email'));
			$mail->setSender($orderDetails['store_name']);
			$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
			$mail->setHtml($html);
			$mail->setText(html_entity_decode($html, ENT_QUOTES, 'UTF-8'));
			$mail->send();
