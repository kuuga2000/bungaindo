...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
		...
		// Update
		//echo "<pre>"; print_r($this->session->data['delivery_date']);
		if (!empty($this->request->post['quantity'])) { //echo "<pre>";print_r($_POST);echo "<pre>"; die();
		...
		else $this->data['delivery_date_pass']=array();
		$this->load->model('catalog/product');
		$deliveryDateError = 0;
		$this->data['error'] = array();
		if (!empty($this->request->post['delivery_date'])) 
			{ 
				foreach ($this->request->post['delivery_date'] as $keyd => $valued) 
				{
				
					$getSpecialProducts =  $this->model_catalog_product->checkProductAvaiableDate($keyd,$valued);
					if(!$getSpecialProducts)
					{
						$deliveryDateError = 1; 
						$this->data['error'][$keyd] = 'Product not available for selected date';
						
					} 
					$this->data['delivery_date_pass'][$keyd]	=  $valued;					
					if(isset($this->request->post['hideSender'][$keyd]))
					{
						$this->session->data['hideSender'][$keyd] = 1;
					}
					else
					{
						$this->session->data['hideSender'][$keyd] = 0;
					}
				}
				
					if($deliveryDateError ==0)
					{
		...
		//print_r($_POST);
		if($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['submit']) && $deliveryDateError == 0)
		{			$this->redirect($this->url->link('checkout/shipping_address', '', 'SSL')); 
		}
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			//echo "<pre>";print_r($this->session->data['cart']);echo "<pre>"; die("tP");
			$products = $this->cart->getProducts();
		//	print_r($products); die("cp");
      		foreach ($products as $product) {
				...
				$specialDaterange = $this->model_catalog_product->getSpecialDates($product['product_id']);
				$startDate = '';
				$endDate =  '';
		...
		//	echo "<pre>";print_r($_SESSION);echo "<pre>"; die("out");
			// Gift Voucher
			...
			$this->data['checkout'] = $this->url->link('checkout/shipping_address', '', 'SSL');
	...
	public function add() {
		...
		if (isset($this->request->post['delivery_date'])) {
				...
				$data = array( $product_id => $delivery_date);
			} else {
				$delivery_date = '';
			}
		$this->load->model('catalog/product');	
			$getSpecialProducts =  $this->model_catalog_product->checkProductAvaiableDate($product_id,$delivery_date); 
			if(!$getSpecialProducts)
			{
				$json['error'] = 'Product not available for selected delivery date.';
			}
		...
		if ($product_info) {			
			...
			if (!$json) {
				...
				$this->session->data['delivery_date'][$product_id]	= $delivery_date;	
	...
	public function quantity_validate()
	{
		...
		$product_discount_query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$id . "'  AND quantity <= '" . (int)$quantity . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity DESC, priority ASC, price ASC LIMIT 1");
		...
		$total_price = $this->currency->format($this->tax->calculate($price, $product_info['tax_class_id'], $this->config->get('config_tax')) * $quantity);
