...
class ModelVendorVendor extends Model {
	...
	*/	
	...
	{
			  $sql = "SELECT COUNT(o.order_id) AS total  FROM " . DB_PREFIX . "order o INNER JOIN " . DB_PREFIX . "order_status os ON os.order_status_id = o.order_status_id WHERE  (os.name LIKE '%Approved%' AND o.shipping_zone_id='".$this->vendor->getRegion()."' AND FIND_IN_SET(o.shipping_city,'".$this->vendor->getCity()."')) OR ((os.name LIKE '%Assigned%' OR os.name LIKE '%Accepted%') AND o.vendor_id = ".(int)$this->vendor->getId().")"; 
			$query =  $this->db->query($sql);
