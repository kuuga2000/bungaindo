...
class ModelVendorVendor extends Model {
	...
	*/	
	...
	{
			 $sql = "SELECT COUNT(o.order_id) AS total  FROM " . DB_PREFIX . "order o INNER JOIN " . DB_PREFIX . "order_status os ON os.order_status_id = o.order_status_id WHERE o.vendor_id = ".(int)$this->vendor->getId()." AND (os.name LIKE '%Approved%' OR os.name LIKE '%Assigned%' OR os.name LIKE '%Accepted%')"; 
			$query =  $this->db->query($sql);
